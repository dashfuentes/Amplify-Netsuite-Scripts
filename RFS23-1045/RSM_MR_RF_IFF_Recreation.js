 ...
 */
define(['N/search', 'N/log', 'N/record', '../lodash'],function(search, log, record, _) {
  /**
  ...
  function getInputData() {
    return search.create({
      ...
      filters: [
        ['type', search.Operator.ANYOF, 'ItemShip'], 'AND',
        ['status', search.Operator.ANYOF, 'ItemShip:A', 'ItemShip:B'], 'AND',
        ['internalid', search.Operator.ANYOF, '6407099', '4847714'], 'AND',
  ...
  function mapStage(context) {
    ...
    try {
      ...
      // Loading old Item Fulfillment
      var oldIFF = record.load({
        type: record.Type.ITEM_FULFILLMENT,
        id: input.id
      });
      log.debug('Original IFF', oldIFF);

      // Creating Fulfillment Request
      // var fr = createFulFillmentRequest(oldIFF);
      // log.debug('Request Fulfillment ID', fr.getValue({ fieldId: 'id' }));
      var newIFF = record.copy({
        type: record.Type.ITEM_FULFILLMENT,
        id: input.id
      });
      var iffid = newIFF.save({ enableSourcing: false, ignoreMandatoryFields: true });
      log.debug('New Item Fulfillment ID', iffid);
  ...
  /**
   * @description Create a new Request Fulfillment based on the Item Fulfillment data provided
